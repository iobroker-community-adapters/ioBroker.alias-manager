...
function initSelectId(callback) {
    setTimeout(function () {
        $('#dialogSelectId').css('z-index', modalZIndexCount++);
    }, 100);
    if (selectId) {
...
function tryParseJSON(jsonString) {
    //Returns parsed object or false, if jsonString is not valid
    try {
...
function multiReplace(string, replacementObj) {
    //Replaces multiple replacements in string. replacementObj = [{searchValue: "", newValue: ""}, ...]
...
function enhanceTextInputToCombobox(targetInput, options, iconsFromOption, onSelect) {
    ...
    $(targetInput).each(function () {
        if (!$(this).parent('div').hasClass('combobox')) {
            ...
            $(this).after(
                `<a class='comboboxDropdownTrigger waves-effect waves-teal btn-small btn-flat' data-target='dropdown_${encodeURIComponent(
                    targetInput,
                )}' href='#' onclick='console.log("Combobox dropdown clicked"); $enhanceTextInputToComboboxActualTarget = $(this).prevAll("input"); enhanceTextInputToComboboxScrollDropdownTo($(this).data("target"), $(this).prevAll("input").val());'><i class='material-icons comboboxDropdownTriggerArrow' style='font-size: 25px;'>arrow_drop_down</i></a>`,
            );
        }
    ...
    options.forEach(function (option) {
        if (option.substring(0, 1) == '[' && option.substr(-1) == ']') {
            ...
            if (icon != '') {
                comboboxContent += `		<img src='${
                    icon
                }' style='display: block; margin-bottom: 5px; min-width: 40px; max-width: 40px; max-height: 40px; width: auto; height: auto;'>`;
            }
            comboboxContent += `			${caption}&nbsp;`;
            comboboxContent += '		</a>';
            comboboxContent += '	</li>';
...
function enhanceTextInputToComboboxEntryToInput(value) {
    ...
    if (decodeURIComponent(value).substring(0, 10) == '[VARIABLE]') {
        ...
        if (variable !== null) {
            value = decodeURIComponent(decodeURIComponent(value).replace('[VARIABLE]', '')).replace(
                '{}',
                `{${variable}}`,
            );
...
function loadAliases(callback) {
    ...
    getAliases('', function (error, result) {
        if (!error && result) {
            ...
            if (error) {
                console.log(`Error getting aliases: ${error}`);
            } else {
...
function getAliases(aliasName, callback) {
    ...
    socket.emit(
        ...
        'system',
        'device',
        { startkey: `alias${aliasName}`, endkey: `alias${aliasName}.\u9999` },
        function (err, res) {
            if (!err && res) {
                                ...
                                function (err, res) {
                                    if (!err && res) {
                                        ...
                                        for (key in result) {
                                            if (
                                                key.split('.').length > 3 &&
                                                typeof result[key.substr(0, key.lastIndexOf('.'))] == udef
                                            ) {
...
function getAliasesMain() {
    ...
    Object.keys(aliases).forEach(function (alias) {
        ...
        if (parentAlias != 'Alias' && parseInt(parentAlias.substr(6)).toString() != parentAlias.substr(6)) {
            aliasesMain.push(parentAlias);
        }
    });
    aliasesMain = removeDuplicates(aliasesMain);
    aliasesMain = aliasesMain.filter(function (element) {
        //Filter for main Aliases (that are elements, that have sub-aliases OR that are direct childs of alias.0. [which means, lastIndexOf(".") == 7])
        if (
            element.lastIndexOf('.') == 7 ||
            aliasesMain.filter(function (_element) {
                return _element.indexOf(`${element}.`) == 0;
            }).length > 0
        ) {
            return true;
        }
...
function getHistoryInstances(callback) {
    (function () {
        //Closure--> (everything declared inside keeps its value as ist is at the time the function is created)
        var _callback = callback;
        var _toDo = function () {
            historyInstances = [];
            for (id in iobrokerObjects) {
                if (
                    id.indexOf('system.adapter.') == 0 &&
                    !isNaN(id.substr(id.lastIndexOf('.') + 1)) &&
                    iobrokerObjects[id] &&
                    iobrokerObjects[id].common &&
                    iobrokerObjects[id].common.type === 'storage'
                ) {
...
function getEnumerationName(enumeration) {
    ...
    if (
        ...
        typeof enumerations[enumeration].common != udef &&
        typeof enumerations[enumeration].common.name != udef
    ) {
        if (
            typeof enumerations[enumeration].common.name == 'object' &&
            typeof enumerations[enumeration].common.name[systemLang] != udef
        ) {
            name = enumerations[enumeration].common.name[systemLang];
        } else if (
            typeof enumerations[enumeration].common.name == 'object' &&
            typeof enumerations[enumeration].common.name['en'] != udef
        ) {
...
function getEnumerationsMain() {
    ...
    if (enumerations) {
        ...
        enumerationsMain = enumerationsMain.filter(function (element) {
            //Filter for main Enumerations (that are elements, that have sub-enumerations)
            if (
                enumerationsMain.filter(function (_element) {
                    return _element.indexOf(`${element}.`) == 0;
                }).length > 0
            ) {
                return true;
        ...
        $('#dialogViewsAutocreateEnumerationMain')
            .empty()
            .append(`<option disabled selected value>${_('Select Enumeration')}</option>`);
        ...
        if (error) {
            ...
            console.log('There are no Enumerations');
        }
        $('#viewsAutocreateButtonProgress').hide();
    }
...
function getDatapointConfiguration(id) {
    if (id && iobrokerObjects[id]) {
        ...
        if (typeof r.name == 'object') {
            r.name = r.name[systemLang] || r.name['de'] || r.name['en'] || Object.keys(r.name)[0] || '';
        }
        ...
        r.min =
            iobrokerObjects[id].common && typeof iobrokerObjects[id].common.min != udef
                ? iobrokerObjects[id].common.min
                : '';
        r.max =
            iobrokerObjects[id].common && typeof iobrokerObjects[id].common.max != udef
                ? iobrokerObjects[id].common.max
                : '';
        r.read =
            iobrokerObjects[id].common && typeof iobrokerObjects[id].common.read != udef
                ? iobrokerObjects[id].common.read
                : true;
        r.write =
            iobrokerObjects[id].common && typeof iobrokerObjects[id].common.write != udef
                ? iobrokerObjects[id].common.write
                : true;
        r.aliasId = id;
        r.aliasRead = '';
        r.aliasWrite = '';
        r.states =
            iobrokerObjects[id].common && typeof iobrokerObjects[id].common.states != udef
                ? iobrokerObjects[id].common.states
                : null;
        r.custom =
            iobrokerObjects[id].common && typeof iobrokerObjects[id].common.custom != udef
                ? iobrokerObjects[id].common.custom
                : null;
        return r;
    }
    return false;
...
function getNewId(aliasPath, id, askForId, excludeList) {
    ...
    if (aliases[`${aliasPath}.${id}`] || excludeList.indexOf(id) > -1) {
        var count = 2;
        while (aliases[`${aliasPath}.${id}_${count}`] || excludeList.indexOf(`${id}_${count}`) > -1) {
            count++;
        }
        id = `${id}_${count}`;
    }
...
function load(settings, onChange) {
    ...
    if (isReact) {
        ...
        customCSS +=
            '.table-values.highlight > tbody > tr:nth-child(2n):hover { background-color: rgba(0,0,0,0.08) !important; }';
        ...
        customCSS +=
            '.m .btn.disabled { background-color: rgba(0, 0, 0, 0.12) !important; color: rgba(0, 0, 0, 0.26) !important; }';
        ...
        customCSS +=
            '.dialog-select-object-ids .material .main-toolbar-table .panel-button { background-color: transparent; }';
        customCSS +=
            '.dialog-select-object-ids .material .main-toolbar-table .panel-button:hover { background-color: rgba(0,0,0,0.08); }';
        ...
        customCSS +=
            '.dialog-select-object-ids table.objects-list-table tr:hover { background-color: #3399cc; outline: none; color: #ffffff; }';
        customCSS += '.dialog-select-object-ids table.objects-list-table tr:hover .fancytree-title { color: #ffffff; }';
        customCSS +=
            '.dialog-select-object-ids table.objects-list-table tr.fancytree-active { background-color: #236b8e !important; outline: none !important; color: #ffffff !important; }';
        customCSS +=
            '.dialog-select-object-ids table.objects-list-table tr.fancytree-active .fancytree-title { color: #ffffff; }';
        ...
        customCSS +=
            '.m [type=checkbox].filled-in:checked+span:not(.lever):after { border: #164477; background-color: #164477; }';
        customCSS +=
            '.m [type=radio].with-gap:checked+span:after, .m [type=radio]:checked+span:after { background-color: #164477; }';
        customCSS +=
            '.m [type=radio].with-gap:checked+span:after, .m [type=radio].with-gap:checked+span:before, .m [type=radio]:checked+span:after { border: 2px solid #164477; }';

        customCSS +=
            '.m.react-dark .btn, .m.react-dark .btn-small, .m.react-dark button.btn i, .m.react-dark button.btn-small i { background-color: #272727; }';
        customCSS +=
            '.m.react-dark .dialog-select-container .main-header-table, .m.react-dark .dialog-select-container .main-header-table tr, .m.react-dark .dialog-select-container .objects-list-table { background: #272727!important; }';
        customCSS +=
            '.m.react-dark .dropdown-content li.grey.lighten-3, .m.react-dark .dropdown-content li.selected, .m.react-dark .dropdown-content li:hover { background-color: #4c4c4c !important; }';
        ...
        if ($('.m.adapter-container').hasClass('react-dark')) {
            $('.m.material-dialogs').addClass('react-dark');
        }
    }
    ...
    var fixHelper = function (e, ui) {
        ui.children().each(function () {
            $(this).width($(this).width()).css({
                'background-color': 'rgba(180,180,180,0.75)',
                'box-shadow': '-5px 5px 5px 0px rgba(180,180,180,0.75)',
            });
        });
        return ui;
    };
    ...
    function getIobrokerObjects() {
        ...
        if (parent && parent.gMain && typeof parent.gMain.objects == 'object') {
            ...
            if (iobrokerObjectsReadyFunctions.length) {
                console.log(
                    'There are some functions that were buffered while fetching the ioBroker Objects. They will be executed now...',
                );
            }
            for (i = 0; i < iobrokerObjectsReadyFunctions.length; i++) {
                if (typeof iobrokerObjectsReadyFunctions[i] == 'function') {
                    iobrokerObjectsReadyFunctions[i]();
                }
            }
    ...
    function loadTabMain() {
        ...
        var aliasesMainTree = aliasesMain.reduce(
            function (total, current) {
                current.split('.').reduce(function (currentTotal, currentValue, currentIndex, currentArray) {
                    ...
                    if (!temp) {
                        currentTotal.children.push((temp = { id: id, label: label }));
                    }
                    return temp;
                }, total);
                return total;
        ...
        function mainCreateNestedAliasesMainList(list) {
            if (list?.length > 0) {
                ...
                list.forEach(function (alias) {
                    ...
                    html +=
                        "		<div class='collapsible-header'><i class='material-icons collapsible-header-inactive indigo-text text-darken-4' style='margin: -9px 6px -9px 0px;'>folder</i><i class='material-icons collapsible-header-active indigo-text text-darken-4' style='margin: -9px 6px -9px 0px;'>folder_open</i>";
                    html += alias.label + (name && name != alias.label ? `<br>(${name})` : '');
                    if (alias.id != 'alias' && alias.id != 'alias.0') {
                        html += `		<a class='waves-effect waves-light btn-floating btn-flat btn-small mainAliasListEditButton' data-alias='${
                            alias.id
                        }' style='margin: -6px; position: absolute; right: 25px;'><i class='material-icons left' style='color: black;'>edit</i></a>`;
                    }
                    html += '		</div>';
                    html += "		<div class='collapsible-body'>";
                    html += mainCreateNestedAliasesMainList(alias.children);
                    var datapoints = Object.keys(aliases).filter(function (element) {
                        return (
                            element.indexOf(alias.id) == 0 &&
                            element.substr(alias.id.length).lastIndexOf('.') == 0 &&
                            aliasesMain.indexOf(element) == -1
                        );
                    });
                    if (datapoints) {
                        html += `		<div style='padding: 10px; overflow-x: auto; cursor: pointer;' class='mainAliasTreeDatapoints' data-alias='${
                            alias.id
                        }'><ul>`;
        ...
        aliasesMain.forEach(function (alias, index) {
            ...
            tableRow += '	</td>';
            tableRow += '	<td>';
            tableRow += `		<a class='waves-effect waves-light btn-floating btn-flat btn-small mainAliasListEditButton' id='mainAliasListTableEditButton_${
                index
            }' data-alias='${alias}'><i class='material-icons left' style='color: black;'>edit</i></a>`;
            tableRow += '	</td>';
            tableRow += '</tr>';
        ...
        $('.mainAliasListEditButton, .mainAliasListTableRow, .mainAliasTreeDatapoints	')
            .off('click')
            .on('click', function () {
                ...
                (function () {
                    ...
                    setTimeout(function () {
                        $('#aliasesSelectedAlias').val(_aliasPath).select().trigger('change');
                    }, 10);
                })(); //<--End Closure
            });
    }
    ...
    function loadTabAliases(showAlias) {
        ...
        $('#aliasesSelectedAlias')
            .empty()
            .append(`<option disabled${showAlias ? '' : ' selected'} value>${_('Select Alias')}</option>`);
        getAliasesMain().forEach(function (alias, index) {
            var name = getAliasName(alias);
            $('#aliasesSelectedAlias').append(
                `<option value='${alias}' data-index='${index}'>${alias}${
    ...
    function aliasesLoadAlias(aliasId) {
        ...
        if (aliasesSelectedAlias) {
            ...
            var datapoints = Object.keys(aliases).filter(function (element) {
                return (
                    element.indexOf(aliasesSelectedAlias) == 0 &&
                    element.substr(aliasesSelectedAlias.length).lastIndexOf('.') == 0 &&
                    aliasesMain.indexOf(element) == -1
                );
            });
    ...
    $('#aliasesNewAlias').on('click', function () {
        ...
        if (aliasId.indexOf('alias.0.') !== 0) {
            aliasId = `alias.0.${aliasId}`;
        }
        if (aliasId.length < 9) {
            return;
        }
    ...
    $('#aliasesNewDatapoint').on('click', function () {
        if (aliasesAddDatapoint()) {
            setTimeout(function () {
                $('.adapter-body').scrollTop($('.adapter-body')[0].scrollHeight);
            }, 500);
        }
    });
    ...
    function aliasesDatapointListAddLine(alias, isMain) {
        ...
        if (aliasId) {
            aliasesUsedAliasIds.push(aliasId);
        }
        listContent += `<li class='aliasesDatapointCollapsibleItem${
            isMain ? ' active' : ''
        }' style='background-color: rgba(255,255,255,0.5); border-width: 4px;' data-id='${alias}' data-main='${
            isMain ? 'true' : 'false'
        }'>`;
        listContent += `<div class='aliasesDatapointCollapsibleHeader collapsible-header' data-id='${
            alias
        }' data-main='${isMain ? 'true' : 'false'}'>`;
        listContent += "<div class='row aliasesDatapointRow' style='background-color: rgba(0,0,0,0.2);'>";
        if (!isMain && aliases[alias].type == 'state') {
            listContent += `<i class='aliasesDatapoint copy material-icons' data-id='${
                alias
            }' data-setting='copy' data-main='${
                isMain ? 'true' : 'false'
            }' style='position:absolute; right:30px; margin-top: 2px; cursor:pointer; color:#000000;'>content_copy</i>`;
        }
        if (!isMain) {
            listContent += `<i class='aliasesDatapoint delete material-icons' data-id='${
                alias
            }' data-setting='delete' data-main='${
                isMain ? 'true' : 'false'
            }' style='position:absolute; right:6px; margin-top: 2px; cursor:pointer; color:#ce0000;'>delete</i>`;
        }
        listContent += "<div class='col s11 m6 l6'>";
        listContent += `<h6><i class='material-icons'>keyboard_arrow_down</i><span class='aliasesDatapoint id' data-id='${
            alias
        }' data-setting='id'>${alias}</span>:`;
        listContent += '</h6>';
        listContent += '</div>';
        listContent += "<div class='col s12 m5 l5'>";
        listContent += `<input class='val aliasesDatapoint' name='aliasesDatapoint_${
            alias
        }_NAME' id='aliasesDatapoint_${alias}_NAME' data-id='${alias}' data-setting='name' value='${name}' data-main='${
            isMain ? 'true' : 'false'
        }'></input>`;
        ...
        listContent += '</div>';
        listContent += '</div>';
        listContent += `<div class='aliasesDatapointCollapsibleBody collapsible-body' data-id='${alias}' data-main='${
            isMain ? 'true' : 'false'
        }'>`;
        ...
        listContent += `<input class='val aliasesDatapoint' name='aliasesDatapoint_${
            alias
        }_ROLE' id='aliasesDatapoint_${alias}_ROLE' data-id='${alias}' data-setting='role' value='${role}' data-main='${
            isMain ? 'true' : 'false'
        }'></input>`;
        ...
        listContent += '</div>';
        if (aliases[alias].type == 'state') {
            //STATE
            listContent += "<div class='col s12 m6 l4'>";
            listContent += `<input class='val aliasesDatapoint' name='aliasesDatapoint_${
                alias
            }_TYPE' id='aliasesDatapoint_${alias}_TYPE' data-id='${alias}' data-setting='type' value='${
                type
            }'></input>`;
            ...
            listContent += `<input class='val aliasesDatapoint' name='aliasesDatapoint_${
                alias
            }_UNIT' id='aliasesDatapoint_${alias}_UNIT' data-id='${alias}' data-setting='unit' value='${
                unit
            }'></input>`;
            ...
            listContent += "<div class='col s12 m6 l4'>";
            listContent += `<input class='val aliasesDatapoint' data-type='number' type='number' name='aliasesDatapoint_${
                alias
            }_MIN' id='aliasesDatapoint_${alias}_MIN' data-id='${alias}' data-setting='min' value='${min}'></input>`;
            ...
            listContent += `<input class='val aliasesDatapoint' data-type='number' type='number' name='aliasesDatapoint_${
                alias
            }_MAX' id='aliasesDatapoint_${alias}_MAX' data-id='${alias}' data-setting='max' value='${max}'></input>`;
            listContent += `<label for='aliasesDatapoint_${alias}_MAX' class='translate'></label>`;
            listContent += "<span class='translate'>common.max</span>";
            listContent += '</div>';
            listContent += "<div class='col s12 m6 l4'>";
            listContent += '<p><label>';
            listContent += `<input class='val aliasesDatapoint checkbox' data-type='checkbox' type='checkbox' name='aliasesDatapoint_${
                alias
            }_READ' id='aliasesDatapoint_${alias}_READ' data-id='${alias}' data-setting='read'${
            ...
            listContent += `<input class='val aliasesDatapoint checkbox' data-type='checkbox' type='checkbox' name='aliasesDatapoint_${
                alias
            }_WRITE' id='aliasesDatapoint_${alias}_WRITE' data-id='${alias}' data-setting='write'${
            ...
            listContent += `<a class='val button aliasesDatapoint btn-floating${
                states ? ' blue' : ''
            }' data-type='button' name='aliasesDatapoint_${alias}_STATES' id='aliasesDatapoint_${
                alias
            }_STATES' data-id='${alias}' data-setting='states'><i class='material-icons'>view_list</i></a>&nbsp;`;
            listContent += `<a class='val button aliasesDatapoint btn-floating${
                custom ? ' blue' : ''
            }' data-type='button' name='aliasesDatapoint_${alias}_CUSTOM' id='aliasesDatapoint_${
                alias
            }_CUSTOM' data-id='${alias}' data-setting='custom'><i class='material-icons'>build</i></a>&nbsp;`;
            listContent += '</div>';
            listContent += '</div>';
            ...
            listContent += `<input class='val aliasesDatapoint' name='aliasesDatapoint_${
                alias
            }_ALIAS_ID' id='aliasesDatapoint_${alias}_ALIAS_ID' data-id='${alias}' data-setting='aliasId' value='${
                typeof aliasId == 'object' ? JSON.stringify(aliasId) : aliasId
            }'></input>`;
            ...
            listContent += `<i class='material-icons aliasesDatapoint selectId' data-id='${
                alias
            }' data-selectidfor='aliasesDatapoint_${
                alias
            }_ALIAS_ID' style='position: absolute; right: 5px; top: 10px; cursor: hand;'>edit</i>`;
            listContent += '</div>';
            listContent += "<div class='col s12 m6 l4'>";
            listContent += `<input class='val aliasesDatapoint' name='aliasesDatapoint_${
                alias
            }_ALIAS_READ' id='aliasesDatapoint_${alias}_ALIAS_READ' data-id='${
                alias
            }' data-setting='aliasRead' value=''></input>`; //Value is set later, because it can contain special chars
            ...
            listContent += `<input class='val aliasesDatapoint' name='aliasesDatapoint_${
                alias
            }_ALIAS_WRITE' id='aliasesDatapoint_${alias}_ALIAS_WRITE' data-id='${
                alias
            }' data-setting='aliasWrite' value=''></input>`; //Value is set later, because it can contain special chars
        ...
        listContent += `<a class='aliasesDatapoint save waves-effect waves-light btn' id='aliasesDatapoint_${
            alias
        }_SAVE' data-id='${alias}' data-setting='save' style='margin: 8px;${
            unsavedNew ? '' : ' display: none;'
        }'><i class='material-icons left'>save</i><span class='translate'>Save changes</span></a>`;
        listContent += '</div>';
        listContent += '</div>';
        if (convertedToNumber) {
            ...
            listContent +=
                "<i class='material-icons left' style='vertical-align: middle; font-size: 1rem; margin-right: 6px;'>info</i><span class='translate'>Converted some objects to type number</span>";
            listContent += '</div>';
            listContent += '</div>';
        }
        if (addedNative) {
            ...
            listContent += "<div class='col s12 m12 l12' style='margin: 2px 0px 2px 8px;'>";
            listContent +=
                "<i class='material-icons left' style='vertical-align: middle; font-size: 1rem; margin-right: 6px;'>info</i><span class='translate'>Added empty native-ojbect to datapoint</span>";
            listContent += '</div>';
            listContent += '</div>';
        }
        listContent += '</div>';
        ...
        $(`.aliasesDatapoint.copy[data-id="${alias}"]`).on('click', function (event) {
            ...
            if (aliases[`${$('#aliasesSelectedAlias').val()}.${id}`]) {
                var count = 2;
                while (aliases[`${$('#aliasesSelectedAlias').val()}.${id}_${count}`]) {
                    count++;
                }
                id = `${id}_${count}`;
            }
            if (
                aliasesAddDatapoint({
                    id: id,
                    name: $(`.aliasesDatapoint.val[data-id="${alias}"][data-setting="name"]`).val(),
                    role: $(`.aliasesDatapoint.val[data-id="${alias}"][data-setting="role"]`).val(),
                    type: $(`.aliasesDatapoint.val[data-id="${alias}"][data-setting="type"]`).val(),
                    unit: $(`.aliasesDatapoint.val[data-id="${alias}"][data-setting="unit"]`).val(),
                    min: $(`.aliasesDatapoint.val[data-id="${alias}"][data-setting="min"]`).val(),
                    max: $(`.aliasesDatapoint.val[data-id="${alias}"][data-setting="max"]`).val(),
                    read: $(`.aliasesDatapoint.val[data-id="${alias}"][data-setting="read"]`).prop('checked'),
                    write: $(`.aliasesDatapoint.val[data-id="${alias}"][data-setting="write"]`).prop('checked'),
                    aliasId: $(`.aliasesDatapoint.val[data-id="${alias}"][data-setting="aliasId"]`).val(),
                    aliasRead: $(`.aliasesDatapoint.val[data-id="${alias}"][data-setting="aliasRead"]`).val(),
                    aliasWrite: $(`.aliasesDatapoint.val[data-id="${alias}"][data-setting="aliasWrite"]`).val(),
                    states: $(`.aliasesDatapoint.val[data-id="${alias}"][data-setting="states"]`).data('value'),
                    custom: $(`.aliasesDatapoint.val[data-id="${alias}"][data-setting="custom"]`).data('value'),
                })
            ) {
                setTimeout(function () {
                    $('.adapter-body').scrollTop($('.adapter-body')[0].scrollHeight);
        ...
        $(`.aliasesDatapoint.delete[data-id="${alias}"]`).on('click', function (event) {
            event.stopPropagation();
            if (confirm(_("Delete datapoint? This can't be undone!"))) {
                aliasesDeleteDatapoints([alias], function () {
        ...
        $(`.aliasesDatapoint.val[data-id="${alias}"][data-setting="states"]`).on('click', function (event) {
            event.stopPropagation();
            initDialog('dialogAliasesShowStates', function () {
                //save dialog
            });
        ...
        $(`.aliasesDatapoint.val[data-id="${alias}"][data-setting="custom"]`).on('click', function (event) {
            event.stopPropagation();
            initDialog('dialogAliasesShowCustom', function () {
                //save dialog
            });
            $('#dialogAliasesShowCustom').data('alias', alias);
            $('#dialogAliasesShowCustomTitle').html(alias);
            $('#dialogAliasesShowCustomTableEdit')
                .off('click')
                .on('click', function () {
                    if (aliases[alias]?.UNSAVED_NEW) {
                        alert('You may need to save it first.');
                    }
        ...
        $(`.aliasesDatapoint.selectId[data-id="${alias}"]`).on('click', function () {
            initDialog('dialogAliasesSelectAliasId', function () {
                //save dialog
                if (
                    $('#tabsDialogAliasesSelectAliasId a.active').attr('href') ==
                    '#dialogAliasesSelectAliasIdSeperateReadWrite'
                ) {
                    //SeperateReadWrite
                    var newId = `{"read": "${$(
                        '#dialogAliasesSelectAliasIdSeperateReadWriteAliasIdRead',
                    ).val()}", "write": "${$('#dialogAliasesSelectAliasIdSeperateReadWriteAliasIdWrite').val()}"}`;
                } else {
                    //OneReadWrite
                    var newId = $('#dialogAliasesSelectAliasIdOneReadWriteAliasId').val();
                }
                $(`#${$('#dialogAliasesSelectAliasId').data('selectidfor')}`)
                    .val(newId)
                    .trigger('change');
            });
            ...
            if (typeof val == 'object' && (val.read || val.write)) {
                //SeperateReadWrite
                M.Tabs.getInstance($('#tabsDialogAliasesSelectAliasId')).select(
                    'dialogAliasesSelectAliasIdSeperateReadWrite',
                );
                ...
                //OneReadWrite
                M.Tabs.getInstance($('#tabsDialogAliasesSelectAliasId')).select(
                    'dialogAliasesSelectAliasIdOneReadWrite',
                );
        ...
        enhanceTextInputToCombobox(
            `.aliasesDatapoint.val[data-id="${alias}"][data-setting="type"]`,
            Object.keys(defaultDatapointRoles).join(';'),
            false,
        );
        $(`.aliasesDatapoint.val[data-id="${alias}"][data-setting="type"]`)
            .on('change init', function () {
                enhanceTextInputToCombobox(
                    `.aliasesDatapoint.val[data-id="${$(this).data('id')}"][data-setting="role"]`,
                    (defaultDatapointRoles[$(this).val()] || []).join(';'),
                );
            })
            .trigger('init');
        enhanceTextInputToCombobox(
            `.aliasesDatapoint.val[data-id="${alias}"][data-setting="role"][data-main="true"]`,
            defaultMainRoles.join(';'),
            false,
        );
        //StopPropagation for aliasesDatapoints
        $('.aliasesDatapoint.val:not(.button)')
            .off('click')
    ...
    $('.dialogAliasesSelectAliasIdAliasesDatapoint.selectId').on('click', function () {
        ...
        initSelectId(function (sid) {
            sid.selectId(
                'show',
                $(`#${$('#dialogSelectId').data('selectidfor')}`).val(),
                { type: 'state' },
                function (newId) {
                    if (newId) {
                        $(`#${$('#dialogSelectId').data('selectidfor')}`)
    ...
    function aliasesAddDatapoint(obj) {
        ...
        if (obj.states) {
            newObj.common.states = obj.states;
        }
        if (obj.custom) {
            newObj.common.custom = obj.custom;
        }
    ...
    function aliasesCopyAlias(deleteOld, saveNew) {
        initDialog('dialogAliasesCopyAlias', function () {
            //save dialog
            ...
            $('#dialogAliasesCopyAliasReplaceDatapointsList > li').each(function () {
                ...
                replacements.push({
                    searchValue: $(`.dialogAliasesCopyAliasReplaceDatapoints.searchvalue[data-index=${index}]`).val(),
                    newValue: $(`.dialogAliasesCopyAliasReplaceDatapoints.newvalue[data-index=${index}]`).val(),
                });
            });
            var idsToSave = [];
            var idsToDelete = [];
            for (oldAlias in aliases) {
                if (oldAlias.indexOf(oldId) == 0) {
                    ...
                    if (oldAlias.length == oldId.length) {
                        ...
                        aliases[newAlias].common.name = ((oldObj.common && oldObj.common.name) || newName).replace(
                            oldName,
                            newName,
                        );
                        if (typeof aliases[newAlias].common.alias != 'object') {
                            aliases[newAlias].common.alias = {};
                        }
                        if (oldObj.common && oldObj.common.alias && oldObj.common.alias.id) {
                            if (typeof oldObj.common.alias.id == 'object') {
                                aliases[newAlias].common.alias.id = JSON.parse(
                                    multiReplace(JSON.stringify(oldObj.common.alias.id), replacements),
                                );
                            } else {
                                aliases[newAlias].common.alias.id = multiReplace(
                                    oldObj.common.alias.id || '',
                                    replacements,
                                );
                            }
                        } else {
        ...
        if (deleteOld) {
            $('#dialogAliasesCopyAlias .modal-action.btn-set span').html(_('Rename this alias'));
        } else {
            $('#dialogAliasesCopyAlias .modal-action.btn-set span').html(_('Copy this alias'));
        }
    ...
    $('#dialogAliasesCopyAliasReplaceDatapointsAdd').on('click', function () {
        ...
        listContent += "<div class='col s12 m5 l5'>";
        listContent += `<input class='val dialogAliasesCopyAliasReplaceDatapoints searchvalue' name='dialogAliasesCopyAliasReplaceDatapoints_${
            index
        }_SEARCH_VALUE' id='dialogAliasesCopyAliasReplaceDatapoints_${index}_SEARCH_VALUE' data-index='${
            index
        }' data-setting='searchvalue' value=''></input>`;
        listContent += `<label for='dialogAliasesCopyAliasReplaceDatapoints_${
            index
        }_SEARCH_VALUE' class='translate'></label>`;
        ...
        listContent += `<input class='val dialogAliasesCopyAliasReplaceDatapoints newvalue' name='dialogAliasesCopyAliasReplaceDatapoints_${
            index
        }_NEW_VALUE' id='dialogAliasesCopyAliasReplaceDatapoints_${index}_NEW_VALUE' data-index='${
            index
        }' data-setting='newvalue' value=''></input>`;
        ...
        listContent += `<i class='material-icons dialogAliasesCopyAliasReplaceDatapoints selectId' data-index='${
            index
        }' data-selectidfor='dialogAliasesCopyAliasReplaceDatapoints_${
            index
        }_NEW_VALUE' style='position: absolute; right: 5px; top: 10px; cursor: hand;'>edit</i>`;
        listContent += '</div>';
        listContent += "<div class='col s1 m1 l1'>";
        listContent += `<i class='material-icons'  style='margin-top:35px; cursor:pointer; color:#e60000;' onclick='$("#dialogAliasesCopyAliasReplaceDatapointsList li[data-index=${
            index
        }]").remove();'>delete</i>`;
        listContent += '</div>';
        listContent += '</div>';
        ...
        enhanceTextInputToCombobox(
            '.dialogAliasesCopyAliasReplaceDatapoints.searchValue',
            aliasesUsedAliasIds.join(';'),
            false,
        );
    ...
    function aliasesSaveDatapoints(ids, callback) {
        ...
        if (alias) {
            ...
            if (typeof newObj.common != 'object') {
                newObj.common = {};
            }
            if (typeof newObj.native != 'object') {
                newObj.native = {};
            }
            if (typeof newObj.common.alias != 'object' && !isMain) {
                newObj.common.alias = {};
            }
            $(`.aliasesDatapoint.val[data-id="${alias}"]`).each(function () {
                ...
                if (setting == 'aliasId') {
                    ...
                    if ($this.val()) {
                        ...
                        delete newObj.common.alias.read;
                    }
                } else if (setting == 'aliasWrite') {
                    if ($this.val()) {
                        newObj.common.alias['write'] = $this.val();
                    } else {
                        delete newObj.common.alias.write;
                    }
            ...
            if (isMain) {
                //Update Id and Name in Selectbox
                var name = $(`.aliasesDatapoint.val[data-id="${alias}"][data-setting="name"]`).val() || alias;
                $(`#aliasesSelectedAlias option[value="${alias}"]`).html(
        ...
        var toDo = function () {
            if (iobrokerObjects[$('#autocreateAliasSourceId').val()]) {
                $('#autocreateAliasSourceIdCommonName').html(
                    iobrokerObjects[$('#autocreateAliasSourceId').val()].common.name,
                );
    ...
    $('#autocreateAliasCreatePreview').on('click', function () {
        ...
        if (!sourceId || typeof iobrokerObjects[sourceId] == udef) {
            return;
        }
        ...
        if (channelDetectorResult) {
            channelDetectorResult.forEach(function (device) {
                //Iterate through all all found device-types
                console.log(`Detected channel type ${device.type}:`);
                if (!channelDetectorUsedDeviceTypeForCommonRole) {
                    //Role was not matched before
                ...
                device.states.forEach(function (state) {
                    //Iterate through all found states
                    if (state.id) {
                        //This is a state with an id
                        r = getDatapointConfiguration(state.id);
                        console.log(`   Detected ${JSON.stringify(r)}`);
                        if (channelDetectorResultDatapointAliasIds.indexOf(state.id) > -1) {
                            //Was matched before
        ...
        for (id in iobrokerObjects) {
            if (
                id.indexOf(sourceId) == 0 &&
                iobrokerObjects[id].type == 'state' &&
                channelDetectorResultDatapointAliasIds.indexOf(id) == -1
            ) {
        ...
        for (i = 0; i < unmatchedChildStates.length; i++) {
            //Try to match unmatchedChildStates
            var stateId = unmatchedChildStates[i];
            r = getDatapointConfiguration(stateId);
            r.deactivateLine = true;
            console.log(`   Unmatched ChildState ${JSON.stringify(r)}`);
            if (channelDetectorResultDatapointAliasIds.indexOf(stateId) > -1) {
                //Was matched before
                console.log('      aliasId was mateched before!');
            }
            if (r.id == 'CONNECTED') {
    ...
    $('#autocreateAliasAddDatapoint').on('click', function () {
        ...
        initSelectId(function (sid) {
            sid.selectId(
                'show',
                autocreateAliasAddDatapointLastSelectId,
                { type: 'state', noMultiselect: false },
                function (newId) {
                    if (newId && iobrokerObjects[newId]) {
                        console.log(iobrokerObjects[newId]);
                        var r = getDatapointConfiguration(newId);
                        ...
                        if (r) {
                            r.id = getNewId($('#autocreateAliasId').val() || '', r.id, false, excludeList);
                        }
    ...
    function autocreateAliasDatapointListAddLine(obj, deactivateLine) {
        ...
        tableLine += `	<td><label><input type='checkbox'${deactivateLine ? '' : " checked='checked'"}' /><span>${
            obj.aliasId
        }</span></label></td>`;
        tableLine += `	<td><input class='autocreateAliasDatapointListVal id' type='text' id='autocreateAliasDatapointListId_${
            index
        }' value='${id}' data-index='${index}'/></td>`;
        tableLine += `	<td><input class='autocreateAliasDatapointListVal name' type='text' id='autocreateAliasDatapointListName_${
            index
        }' value='${obj.name}' data-index='${index}'/></td>`;
        tableLine += '	<td>';
        tableLine += `		<a class='autocreateAliasDatapointListDelete btn-flat' onclick='$(".autocreateAliasDatapointListLine[data-index=${
            index
        }]").remove();'><i class='material-icons'>delete</i></a>`;
        tableLine += '	</td>';
        tableLine += '</tr>';
        $('#autocreateAliasDatapointListTableBody').append(tableLine);
        $('#autocreateAliasDatapointListTableBody')
            .find(`.autocreateAliasDatapointListLine[data-index="${index}"]`)
            .data('object', obj);
        index++;
        $('#autocreateAliasDatapointListTableBody').data('length', index);
        $('.autocreateAliasSaveAll').show();
        //Show save button on change
        $('.autocreateAliasVal, .autocreateAliasDatapointListLine')
            .off('input change')
    ...
    function autocreateAliasSaveAll() {
        var toDo = function () {
            ...
            if (aliasPath.substr(0, 8) != 'alias.0.') {
                if (!confirm("Warning: Alias ID does not start with 'alias.0', continue anyway?")) {
                    return false;
                }
            }
            if (iobrokerObjects[aliasPath]) {
                if (!confirm(`Warning: Alias ID '${aliasPath}' exists. Overwrite?`)) {
                    return false;
                }
            }
            var ids = ['*MAIN*'];
            $('.autocreateAliasDatapointListLine').each(function () {
                if ($(this).find('input[type=checkbox]').prop('checked')) {
                    ids.push($(this).find('.autocreateAliasDatapointListVal.id').val());
                }
            });
    ...
    function autocreateAliasSave(aliasPath, ids, callback) {
        ...
        if (alias) {
            ...
            if (isMain) {
                ...
                if (lineObject.states) {
                    newObj.common.states = lineObject.states;
                }
                if ($('#autocreateAliasOptionsAdoptCommonCustom').prop('checked') && lineObject.custom) {
                    var removeHistoryFrom = $(
                        'input[name=autocreateAliasOptionsAdoptCommonCustomRemoveHistoryFrom]:checked',
                    ).val();
                    ...
                    if (Object.keys(lineObject.custom).length) {
                        newObj.common.custom = lineObject.custom;
                    }
                }
            }
            (function () {
    ...
    function loadTabRenameAliases() {
        ...
        values2table(
            'tableRenameAliasesNamePattern',
            renameAliasesNamePattern,
            onChange,
            onTableRenameAliasesNamePatternReady,
        );
        //Get Aliases and add them to Tree
        var aliasesMain = getAliasesMain();
        var aliasesMainTree = aliasesMain.reduce(
        ...
        function renameAliasesCreateNestedAliasesMainList(list) {
            if (list?.length > 0) {
                ...
                list.forEach(function (alias) {
                    ...
                    if (datapoints) {
                        html += `		<div style='padding: 10px; overflow-x: auto; cursor: pointer;' class='renameAliasesAliasTreeDatapoints' data-alias='${
                            alias.id
                        }'><ul>`;
                        html += `			<li>${datapoints.join('</li><li>')}`;
                        html += '		</ul></div>';
